...
def search(page):
    ...
    if args['object'] == '帖子':
        ...
        pagination = IdleItems.query.filter(
            or_(IdleItems.title.ilike('%' + args['keyword'] + '%') if args['keyword'] is not None else '',
                IdleItems.descriptions.ilike('%' + args['keyword'] + '%') if args['keyword'] is not None else '')
        ).order_by(IdleItems.add_time.desc()).paginate(page=page, per_page=current_app.config['PER_PAGE'], error_out=False)
        posts = pagination.items
        return render_template('search.html', idle_items_count=idle_items_count, pagination=pagination, posts=posts, args=args)
    else:
        ...
        pagination = User.query.filter(User.username.ilike('%' + args['keyword'] + '%')). \
            order_by(User.last_seen.desc()).paginate(page=page, per_page=current_app.config['PER_PAGE'], error_out=False)
        posts = pagination.items
